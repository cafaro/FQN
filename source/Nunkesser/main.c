...
int checkForOutlier(double middle, long seqNo, double median, double kth_value, double QnScaleFactor, log_point *outliers, log_point *inliers){
    ...
    //deviation from median of the item in the center of the window
    //double middle_dev = fabs(middle - median);
    //Threshold
    //double threshold = 3*(o->QnScaleFactor)*kth_value;
    //fprintf(stderr, "Window Middle %.1f (%ld) \t Window Median %.1f\n", middle, seqNo, median);
    //fprintf(stderr, "Middle dev %.1f \tthreshold %.1f\n", middle_dev, threshold);
...
int main (int argc, char *argv[]) {
    ...
    qn0_node = compute_qn0_offline(X, Y, B, w);
    ...
    int seq = k;
    double qn0 = ((BNode *)(qn0_node)->key)->value;
...
#ifndef CSV
    printf("\n\nTest completed successfully! Time for processing %d items: %.3f seconds. Updates/second: %.3f\n\n", (seq_no-w), upd_time/1000, (double)(seq_no-w)/(upd_time/1000));
#else
    fprintf(stderr, "%s,%d,%d,%.5f,%.5f,%d,%d\n",argv[1], seq_no-w, k, upd_time/1000, (seq_no-w)/(upd_time/1000), num_outliers, (seq_no-w)-num_outliers);
#endif
