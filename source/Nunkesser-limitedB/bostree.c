...
static void _bostree_print_helper(BOSNode *node) {
    ...
    snprintf(nodekey, 30, "%ld", ((NodeData*)node->data)->seqNo);
	fprintf(stderr,"  %s [label=\"\\N (%.3f,%d,%d,%d)\"];\n", nodekey, *(double*)node->key, node->left_child_count, node->right_child_count, node->depth);
	if(node->parent_node) {
        snprintf(parentkey, 30, "%ld", ((NodeData*)node->parent_node->data)->seqNo);
        printf("  %s -> %s [color=green];\n", nodekey, parentkey);
	}
	if(node->left_child_node != NULL) {
        snprintf(leftkey, 30, "%ld", ((NodeData*)node->left_child_node->data)->seqNo);
        fprintf(stderr,"  %s -> %s\n", nodekey, leftkey);
		_bostree_print_helper(node->left_child_node);
	}
	if(node->right_child_node != NULL) {
        snprintf(rightkey, 30, "%ld", ((NodeData*)node->right_child_node->data)->seqNo);
        fprintf(stderr,"  %s -> %s\n", nodekey, rightkey);
...
int search_for_j(BOSTree *tree, void *item_key) {
    BOSNode *node = tree->root_node;
    BOSNode *prev_node = NULL;
    int n = bostree_node_count(tree);
    while (node) {
        int cmp = tree->cmp_function(node->key, item_key);
        if (cmp == 0) {
            while (node != NULL && cmp == 0) {
                prev_node = node;
                node = bostree_previous_node(node);
                if (node)
                    cmp = tree->cmp_function(node->key, item_key);
            }
            return bostree_rank(prev_node);
        } else if (cmp > 0) {
            prev_node = node;
            node = node->left_child_node;
        } else if (cmp < 0) {
    ...
    node = prev_node;
    while (node) {
        if (tree->cmp_function(node->key, item_key) > 0)
            return bostree_rank(node);
        node =  node->parent_node;
    }
    return n;
}

int search_for_l(BOSTree *tree, void *item_key) {
    BOSNode *node = tree->root_node;
    BOSNode *prev_node = NULL;
    int n = bostree_node_count(tree);
    while (node) {
